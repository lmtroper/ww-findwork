  ...
  Object.keys(data).forEach((key) => {
    if (key.toLowerCase().includes("job") && key !== "VIEWED_JOBS") {
      ...
      if (jobPostingInfo) {
        ...
        var targetedDegrees =
          jobPostingInfo["Targeted Degrees and Disciplines"] || "N/A";
        var formattedTargetedDegrees =
          extractThemesFromPrograms(targetedDegrees); // Pass targetedDegrees
        var region = jobPostingInfo["Region"];
        var mappedRegion = mapRegion(region) || "N/A";
        var url = "";
        if (key.toLowerCase().includes("coop")) {
          url = `https://waterlooworks.uwaterloo.ca/myAccount/co-op/full/jobs.htm?ck_jobid=${jobId}`;
        } else if (key.toLowerCase().includes("fulltime")) {
          url = `https://waterlooworks.uwaterloo.ca/myAccount/graduating/jobs.htm?ck_jobid=${jobId}`;
        }
...
function getAverageWorkTermRating(divisionId, wworksData) {
  ...
  for (let key in wworksData) {
    if (key.includes(divisionId)) {
      ...
      if (divisionData && divisionData.length == 5) {
